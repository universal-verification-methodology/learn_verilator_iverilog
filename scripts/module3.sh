...
# Module 3: Testbench Fundamentals (Verilog and C++) Orchestrator
...
# Options
RUN_VERILOG_TESTBENCHES=true
RUN_CPP_TESTBENCHES=true
RUN_COMPARISON=true
...
This script runs examples and tests for Module 3, comparing both paradigms.

OPTIONS:
    Examples:
        --verilog-testbenches    Run Verilog testbench examples
        --cpp-testbenches        Run C++ testbench examples
        --comparison             Show comparison guide
...
EXAMPLES:
    # Run all examples and tests (both paradigms)
    $0
    
    # Run only Verilog testbenches
    $0 --verilog-testbenches
...
check_prerequisites() {
    ...
    if [[ $missing_tools -gt 0 ]]; then
        print_status $RED "Please install missing tools before running Module 3 examples."
        exit 1
    fi
...
run_test_dir() {
    ...
    if [[ $exit_code -eq 0 ]]; then
        ...
        print_status $YELLOW "Check $MODULE3_DIR/tests/$test_dir/test.log for details"
        return 1
    fi
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --verilog-testbenches)
            RUN_VERILOG_TESTBENCHES=true
            RUN_CPP_TESTBENCHES=false
            RUN_COMPARISON=false
            shift
            ;;
        --cpp-testbenches)
            RUN_VERILOG_TESTBENCHES=false
            RUN_CPP_TESTBENCHES=true
            RUN_COMPARISON=false
            shift
            ;;
        ...
        --check)
            ...
            media_module_check "$PROJECT_ROOT" 3
            exit $?
            ;;
        --help|-h)
            show_usage
...
main() {
    ...
    print_header "Module 3: Testbench Fundamentals (Verilog and C++)"
    ...
    # Run examples
    if [[ "$RUN_VERILOG_TESTBENCHES" == true ]]; then
        run_example_dir "verilog_testbenches" "Verilog Testbench Examples" || failed=$((failed + 1))
        # Track VCD files
        local vcd1="$MODULE3_DIR/examples/verilog_testbenches/counter_test.vcd"
        if [[ -f "$vcd1" ]]; then
            vcd_file_to_open="$vcd1"
        fi
    fi
    
    if [[ "$RUN_CPP_TESTBENCHES" == true ]]; then
        run_example_dir "cpp_testbenches" "C++ Testbench Examples" || failed=$((failed + 1))
    fi
    
    if [[ "$RUN_COMPARISON" == true ]]; then
        print_header "Comparison Guide"
        if [[ -f "$MODULE3_DIR/examples/comparison/comparison_guide.md" ]]; then
            print_status $BLUE "Comparison guide available at:"
            print_status $BLUE "  $MODULE3_DIR/examples/comparison/comparison_guide.md"
            print_status $YELLOW "View the guide to understand differences between Verilog and C++ testbenches"
        fi
    fi
    
    # Run tests
    if [[ "$RUN_VERILOG_TESTS" == true ]]; then
        if [[ -d "$MODULE3_DIR/tests/verilog_tests" ]]; then
    ...
    if [[ "$RUN_CPP_TESTS" == true ]]; then
        if [[ -d "$MODULE3_DIR/tests/cpp_tests" ]]; then
    ...
    if [[ $failed -eq 0 ]]; then
        ...
        print_status $BLUE "Compare Verilog and C++ testbenches side-by-side to understand the differences"
        echo ""
