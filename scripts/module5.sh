...
# Module 5: Procedural Testbench Writing Orchestrator
...
# Options
RUN_PROCEDURAL_CONSTRUCTS=true
RUN_TIMING_CONTROL=true
RUN_TEST_SEQUENCES=true
RUN_REUSABLE_ROUTINES=true
RUN_FILE_IO=true
RUN_ADVANCED_PATTERNS=false  # Default false (advanced topics)
...
OPTIONS:
    Examples:
        --procedural-constructs   Run procedural constructs examples
        --timing-control          Run timing control examples
        --test-sequences          Run test sequence examples
        --reusable-routines       Run reusable routines examples
        --file-io                 Run file I/O examples
        --advanced-patterns       Run advanced procedural patterns
...
EXAMPLES:
    ...
    # Run only file I/O examples
    $0 --file-io
    
    # Run only reusable routines
    $0 --reusable-routines
...
check_prerequisites() {
    ...
    if [[ $missing_tools -gt 0 ]]; then
        print_status $RED "Please install missing tools before running Module 5 examples."
        exit 1
    fi
...
run_example_dir() {
    ...
    if [[ $exit_code -eq 0 ]]; then
        ...
        print_status $YELLOW "Check $MODULE5_DIR/examples/$example_dir/run.log for details"
        return 1
    fi
}
...
run_test_dir() {
    ...
    if [[ $exit_code -eq 0 ]]; then
        ...
        print_status $YELLOW "Check $MODULE5_DIR/tests/$test_dir/test.log for details"
        return 1
    fi
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --procedural-constructs)
            RUN_PROCEDURAL_CONSTRUCTS=true
            RUN_TIMING_CONTROL=false
            RUN_TEST_SEQUENCES=false
            RUN_REUSABLE_ROUTINES=false
            RUN_FILE_IO=false
            shift
            ;;
        --timing-control)
            RUN_PROCEDURAL_CONSTRUCTS=false
            RUN_TIMING_CONTROL=true
            RUN_TEST_SEQUENCES=false
            RUN_REUSABLE_ROUTINES=false
            RUN_FILE_IO=false
        ...
        --check)
            ...
            media_module_check "$PROJECT_ROOT" 5
            exit $?
            ;;
        --help|-h)
            show_usage
...
main() {
    ...
    print_header "Module 5: Procedural Testbench Writing"
    ...
    # Run examples
    if [[ "$RUN_PROCEDURAL_CONSTRUCTS" == true ]]; then
        if [[ -d "$MODULE5_DIR/examples/procedural_constructs" ]]; then
            run_example_dir "procedural_constructs" "Procedural Constructs Examples" || failed=$((failed + 1))
        else
            print_status $YELLOW "Procedural constructs examples directory not found (optional)"
        fi
    fi
    
    if [[ "$RUN_TIMING_CONTROL" == true ]]; then
        run_example_dir "timing_control" "Timing Control Examples" || failed=$((failed + 1))
        local vcd="$MODULE5_DIR/examples/timing_control/timing_control.vcd"
        if [[ -f "$vcd" ]] && [[ -z "$vcd_file_to_open" ]]; then
            vcd_file_to_open="$vcd"
        fi
    fi
    
    if [[ "$RUN_TEST_SEQUENCES" == true ]]; then
        if [[ -d "$MODULE5_DIR/examples/test_sequences" ]]; then
            run_example_dir "test_sequences" "Test Sequence Examples" || failed=$((failed + 1))
        else
            print_status $YELLOW "Test sequence examples directory not found (optional)"
        fi
    fi
    
    if [[ "$RUN_REUSABLE_ROUTINES" == true ]]; then
        run_example_dir "reusable_routines" "Reusable Routines Examples" || failed=$((failed + 1))
        local vcd="$MODULE5_DIR/examples/reusable_routines/task_function_test.vcd"
        if [[ -f "$vcd" ]] && [[ -z "$vcd_file_to_open" ]]; then
            vcd_file_to_open="$vcd"
        fi
    fi
    ...
    if [[ "$RUN_ADVANCED_PATTERNS" == true ]]; then
        if [[ -d "$MODULE5_DIR/examples/advanced_patterns" ]]; then
            run_example_dir "advanced_patterns" "Advanced Procedural Patterns" || failed=$((failed + 1))
        else
            print_status $YELLOW "Advanced patterns directory not found (optional)"
        fi
    fi
    
    # Run tests
    if [[ "$RUN_VERILOG_TESTS" == true ]]; then
        if [[ -d "$MODULE5_DIR/tests/verilog_tests" ]]; then
    ...
    if [[ "$RUN_CPP_TESTS" == true ]]; then
        if [[ -d "$MODULE5_DIR/tests/cpp_tests" ]]; then
