...
# Module 7: Coverage and Assertions Orchestrator
...
RUN_FUNCTIONAL_COVERAGE=true
RUN_ASSERTIONS=true
RUN_COVERAGE_ANALYSIS=true
RUN_ASSERTION_LIBRARIES=true
...
This script runs examples and tests for Module 7.
Focuses on basic coverage and assertions that work with both tools.

OPTIONS:
    Examples:
        --code-coverage          Run code coverage examples
        --functional-coverage    Run functional coverage examples
        --assertions             Run assertion examples
        --coverage-analysis      Run coverage analysis examples
        --assertion-libraries    Run assertion library examples
...
EXAMPLES:
    ...
    # Run only code coverage examples
    $0 --code-coverage
    
    # Run only assertion examples
    $0 --assertions
...
check_prerequisites() {
    ...
    if [[ $missing_tools -gt 0 ]]; then
        print_status $RED "Please install missing tools before running Module 7 examples."
        exit 1
    fi
...
run_example_dir() {
    ...
    if [[ $exit_code -eq 0 ]]; then
        ...
        print_status $YELLOW "Check $MODULE7_DIR/examples/$example_dir/run.log for details"
        return 1
    fi
}
...
run_test_dir() {
    ...
    if [[ $exit_code -eq 0 ]]; then
        ...
        print_status $YELLOW "Check $MODULE7_DIR/tests/$test_dir/test.log for details"
        return 1
    fi
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --code-coverage)
            RUN_CODE_COVERAGE=true
            RUN_FUNCTIONAL_COVERAGE=false
            RUN_ASSERTIONS=false
            RUN_COVERAGE_ANALYSIS=false
            RUN_ASSERTION_LIBRARIES=false
            shift
            ;;
        --functional-coverage)
        ...
        --check)
            ...
            media_module_check "$PROJECT_ROOT" 7
            exit $?
            ;;
        --help|-h)
            show_usage
...
main() {
    ...
    print_header "Module 7: Coverage and Assertions"
    ...
    if [[ "$RUN_CODE_COVERAGE" == true ]]; then
        run_example_dir "code_coverage" "Code Coverage Examples" || failed=$((failed + 1))
    fi
    
    if [[ "$RUN_FUNCTIONAL_COVERAGE" == true ]]; then
        run_example_dir "functional_coverage" "Functional Coverage Examples" || failed=$((failed + 1))
    fi
    
    if [[ "$RUN_ASSERTIONS" == true ]]; then
        run_example_dir "assertions" "Assertion Examples" || failed=$((failed + 1))
        local vcd="$MODULE7_DIR/examples/assertions/basic_assertions.vcd"
    ...
    if [[ "$RUN_COVERAGE_ANALYSIS" == true ]]; then
        if [[ -d "$MODULE7_DIR/examples/coverage_analysis" ]]; then
            run_example_dir "coverage_analysis" "Coverage Analysis Examples" || failed=$((failed + 1))
        else
            print_status $YELLOW "Coverage analysis examples directory not found (optional)"
        fi
    fi
    
    if [[ "$RUN_ASSERTION_LIBRARIES" == true ]]; then
        run_example_dir "assertion_libraries" "Assertion Library Examples" || failed=$((failed + 1))
    fi
    
    # Run tests
    if [[ "$RUN_VERILOG_TESTS" == true ]]; then
        if [[ -d "$MODULE7_DIR/tests/verilog_tests" ]]; then
    ...
    if [[ "$RUN_CPP_TESTS" == true ]]; then
        if [[ -d "$MODULE7_DIR/tests/cpp_tests" ]]; then
